#!/bin/bash
# Tier 5 / kvcache_fp8_skip23 -- diffusion KV-cache FP8, MIDDLE, on the current
# best config (Candidate A + Cache-DiT).
...
#
# Fewer guards than _skip45: skip-steps 0-2, skip-layers 0 (first 3 steps and first
# 1 block stay BF16). More FP8 coverage -> a bit more memory saved, slightly higher
# quality risk. Between _skip45 (conservative) and _noskip (aggressive).
# Quality gate REQUIRED vs attn_flash (BF16 KV).
...
echo "Starting MiniMax-H3 FL2VA Tier 5 / kvcache_fp8_skip23 (middle, +Cache-DiT) on port $PORT ..."
...
vllm serve /data/models/modelscope/MiniMax/MiniMax-H3/FL2VA \
  ...
  --diffusion-kv-cache-skip-steps '0-2' \
  --diffusion-kv-cache-skip-layers '0' \
