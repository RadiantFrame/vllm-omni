...
> 推导自 4×H800 实践赢家（[`../4h800/`](../4h800/) `deploy_tier4_4b.sh`，114.77s@768p/8s）。
...
| 4 路并发（`deploy_4svc.sh`，4×2 卡） | 稳态 **≈27.25s/路（仅 +2.8%）**，聚合 **8.81 视频/分（效率 97%）**；**同机吞吐最优方案，胜过 8×1 卡**（见 §5） |
...
| `deploy_4svc.sh` | ✅ 吞吐版 | 8 卡起 4 路（每路 2 卡 = deploy.sh 配置，端口 9000–9003，master 29500+i 防端口竞态） |
| `deploy_usp2.sh` | 🗄️ 存档 | USP2 实验版，已实测双输（见 §4），保留作负结果记录 |
...
| USP2（deploy_usp2.sh） | 480p 同上 | ⚠️ 稳态 29.1s（8 次 28.97–29.21），denoise ~0.58s/it | 模型 65.8 GiB；**运行峰值 ~73GB（余量仅 ~8GB）** | `logs/h3_0818_2h800.log` |
| USP2 | 768p/1344×768/8s | ❌ 请求#1 勉强 229.8s（含 warmup），**请求#2 CUDA OOM**（占用 78.79GB、418MB 分配失败，挂 Ulysses all-to-all 输出 reshape） | 模型 67.2 GiB | `logs/h3_0818_2h800_768p.log` |
...
压测口径：`NUM_SERVICES=4 PORT_BASE=9000 bash ../../generate/generate_480p_5s_nsvc.sh`（4 路同时发同一请求，共 8 轮；日志 `logs/deploy_4svc.svc<N>.gpu<..>.log`，0 报错）。
...
**并发衰减极小**：单路 26.5s → 4 路并发 27.25s，**仅 +2.8%**（剔除第 6 轮异常 ≈27.04s，+2.0%）；聚合 **8.81 视频/分**，线性效率 **97%**。TP2 权重常驻 GPU、无 PCIe swap，4 路只共享 NVLink/主机侧带宽 → 几乎不受并发影响。
