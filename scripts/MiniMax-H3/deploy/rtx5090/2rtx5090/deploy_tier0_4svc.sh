...
# deploy_tier0_4svc.svc<N>.gpu<..>.log in LOG_DIR.
# PIDs are written to deploy_tier0_4svc.pids for `kill $(cat ...)`.
...
PID_FILE="$LOG_DIR/deploy_tier0_4svc.pids"
...
for i in "${!PAIRS[@]}"; do
    gpus="${PAIRS[$i]}"
    port=$((PORT_BASE + i))
    log="$LOG_DIR/deploy_tier0_4svc.svc${i}.gpu${gpus}.log"
...
echo ""
echo "Tail a log:   tail -f $LOG_DIR/deploy_tier0_4svc.svc0.gpu0,1.log"
