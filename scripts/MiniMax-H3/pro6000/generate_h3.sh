...
response=$(
  curl --fail-with-body --silent --show-error \
    --request POST \
    --url http://localhost:30052/v1/videos \
...
video_id=$(printf '%s\n' "$response" | jq -er '.id')
# Poll until the async job finishes (POST /v1/videos returns immediately;
# the video is still being generated in the background).
while :; do
  status=$(curl --fail-with-body --silent --show-error \
    --request GET \
    --url "http://localhost:30052/v1/videos/$video_id" | jq -er '.status')
  case "$status" in
    completed) break ;;
    failed) echo "generation failed for video $video_id" >&2; exit 1 ;;
    *) sleep 2 ;;
  esac
done
...
curl --fail-with-body --silent --show-error \
  --request GET \
  --url "http://localhost:30052/v1/videos/$video_id/content" \
  --output fl2va.mp4