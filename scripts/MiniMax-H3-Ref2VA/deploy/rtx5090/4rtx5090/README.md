...
| 脚本 | 结果 | 关键数据 |
|---|---|---|
| `deploy.sh`(TP4+DLO+BF16) | ✅ **可跑** | 峰值 HBM **~10G/卡**(GPU 大量闲余,见下);去噪延迟未记录,建议用 `generate/generate_480p_fanout.sh` 实测(预期优于 2 卡的 1.72s/step) |
...
请求客户端在 `../../generate/`(`generate_480p_fanout.sh` 默认打 8000)。
两脚本均支持 `PROFILER=1` 开关(`--enable-diffusion-pipeline-profiler`)。

## 六、剩余杠杆(按 ROI)

1. **补去噪延迟实测**:同 prompt/seed 跑 `generate_480p_fanout.sh`,与 2 卡 1.72s/step 对照,量化 TP4 收益。
